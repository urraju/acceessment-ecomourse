import logo from '../assets/logo/Alzaf Logo Size 300x300 pixel 1.png'
import logo2 from '../assets/navbarIcon/Vector (2).png'
import search from '../assets/navbarIcon/Search 2.png'
import group from '../assets/navbarIcon/Group.png'
import shopping from '../assets/navbarIcon/Vector (1).png'
import love from '../assets/navbarIcon/Vector.png'
const Navbar = () => {
    return(
        <div>
             <div className='max-w-screen-2xl mx-auto py-3 px-3 flex justify-between gap-5'>
                    <img src={logo} alt="" />
                    <div className='bg-[#EFF0F5] w-1/2 relative px-3 rounded-lg flex justify-between'>
                        <input className='bg-transparent outline-none ' type="text" name="" id="" placeholder='Search Product' />
                        <img className='bg-orange-500 px-3 right-0 absolute py-3 rounded-r-lg' src={search} alt="" />
                    </div>
                    <div className='flex gap-5'>
                        <img className='bg-[#EFF0F5] p-3 rounded-xl' src={group} alt="" />
                        <img className='bg-[#EFF0F5] p-3 rounded-xl' src={shopping} alt="" />
                        <img className='bg-[#EFF0F5] p-3 rounded-xl' src={love} alt="" />
                        <div className='flex bg-orange-400 rounded-lg  '>
                            <img className='w-10' src={logo2} alt="" />
                            <p className='py-2 rounded-2xl px-2 font-inter inline-block font-bold text-white tracking-widest'>Cloud Service</p>
                        </div>
                    </div>
             </div>
        </div>
    )}
export default Navbar;